...
class RtmClient:
    ...
    def __init__(self, auth_response_data: dict):
        ...
        Args
        ----
        auth_response_data : dict
            Dictionary with APP_ID, CHANNEL_NAME, RTM_TOKEN, USERID, BOT_UID.

        Raises
        ------
        ConfigurationError
            If required auth data is missing.
        ...
        # Peer messages go to the bot's UID (the robot); SDK uses sendMessageToPeer(botUid)
        bot_uid = auth_response_data.get("BOT_UID")
        channel = self.channel or ""
        self.destination = (
            str(bot_uid) if bot_uid is not None and str(bot_uid).strip()
            else channel.replace("sdk_", "", 1)
        )
    ...
    def send_message(self, message: dict) -> bool:
        ...
        Note
        ----
        Errors are logged but exceptions are not raised. Callers can check return value.
        ...
        try:
            ...
            base = "https://api.agora.io/dev/v2/project"
            url = f"{base}/{self.app_id}/rtm/users/{self.uid}/peer_messages"
            headers = {
            ...
            if response.status_code == 200:
                return True
            # Log so operators see why RTM failed (e.g. 401, 404, 429)
